...
#ifdef DMI
	// Indicate that DMI is supported
...
inline bool lt_target::get_direct_mem_ptr(tlm::tlm_generic_payload& trans, tlm::tlm_dmi& dmi){
  ...
  if (mem_size < trans.get_address()) {
	  dmi.allow_none();
	  dmi.set_start_address((sc_dt::uint64)0x0);
	  dmi.set_end_address((sc_dt::uint64) (mem_size - 4));
	  return false;
  }
  else {
	  ...
	  dmi.set_start_address((sc_dt::uint64)0x0);
	  dmi.set_end_address((sc_dt::uint64) (mem_size - 4));
	  dmi.set_dmi_ptr((unsigned char*)(&mem[0]));
	  dmi.set_read_latency(rd_latency);
	  dmi.set_write_latency(wr_latency);
	  return true;
  }
  return false;
}
